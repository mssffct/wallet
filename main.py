...
class Main(tk.Frame):
	...
	def init_main(self):
		...
		btn_open_dialog = tk.Button(toolbar, text='Добавить позицию',
									...
									bg="#fff", bd=0, compound=tk.TOP, image=self.add_img)
		btn_open_dialog.pack(side=tk.LEFT)

		self.update_img = tk.PhotoImage(file='update.png')
		btn_edit_dialog = tk.Button(toolbar, text='Редактировать', bg='#fff', bd=0, image=self.update_img,
									compound=tk.TOP, command=self.open_update_dialog)
		btn_edit_dialog.pack(side=tk.LEFT)

		self.delete_img = tk.PhotoImage(file='delete.png')
		btn_delete = tk.Button(toolbar, text='Удалить позицию', bg='#fff', bd=0, image=self.delete_img,
...
#	def fin_summ(self):
#		self.db.c.execute('''SELECT SUM(total) FROM finance as fin_summ''')
...
class Child(tk.Toplevel): # Окно инструментов
	def __init__(self):
		super().__init__(root)
		self.init_child()
		self.view = app
	...
	def init_child(self): 
		...
		self.focus_set() # Ударжание фокуса
